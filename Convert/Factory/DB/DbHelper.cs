...
namespace LeaRun.DataAccess
{
    ...
    public  class DbHelper
    {
        ...
        /// 调试日志
        /// </summary>
        public  LogHelper log = LogFactory.GetLogger(typeof(DbHelper));
        ...
        public DbHelper(SqlInfor model)
        {
            if (model.DbType == 1)
            {
                ConnectionString = string.Format("Data Source={0};Initial Catalog={1};Persist Security Info=True;User ID={2};Password={3}", model.Server, model.DbName, model.UserCard, model.UserPwd);
                this.DatabaseTypeEnumParse("System.Data.SqlClient");
            }
            else if (model.DbType == 0)
            {
                ConnectionString = string.Format("Data Source={0};User ID={1};Password={2 };", model.Server, model.UserCard, model.UserPwd);
        ...
        /// <param name="parameters">执行命令所需的sql语句对应参数</param>
        /// <returns></returns>
        public  int ExecuteNonQuery(DbConnection connection, CommandType cmdType, string cmdText, params DbParameter[] parameters)
        {
            int num = 0;
            try
            {
        ...
        public  IDataReader ExecuteReader(DbTransaction isOpenTrans, CommandType cmdType, string cmdText, params DbParameter[] parameters)
        {
            DbCommand cmd = DbFactory.CreateDbCommand();
        ...
        /// <summary>
        /// 查询数据填充到数据集DataTable中
        /// </summary>
        /// <param name="cmdType">执行命令的类型（存储过程或T-SQL，等等）</param>
        /// <param name="cmdText">命令文本</param>
        /// <returns>数据集DataTable对象</returns>
        ...
        /// <param name="parameters">执行命令所需的sql语句对应参数</param>
        /// <returns>返回一个对象，使用Convert.To{Type}将该对象转换成想要的数据类型。</returns>
        public  object ExecuteScalar(DbConnection connection, CommandType cmdType, string cmdText, params DbParameter[] parameters)
        {
            try
            {
                DbCommand cmd = DbFactory.CreateDbCommand();
        ...
        /// <param name="parameters">执行命令所需的sql语句对应参数</param>
        /// <returns>返回一个对象，使用Convert.To{Type}将该对象转换成想要的数据类型。</returns>
        public  object ExecuteScalar(DbConnection conn, DbTransaction isOpenTrans, CommandType cmdType, string cmdText)
        {
            try
            {
                DbCommand cmd = DbFactory.CreateDbCommand();
        ...
        /// <param name="parameters">执行命令所需的sql语句对应参数</param>
        /// <returns>返回一个对象，使用Convert.To{Type}将该对象转换成想要的数据类型。</returns>
        public  object ExecuteScalar(DbTransaction isOpenTrans, CommandType cmdType, string cmdText, params DbParameter[] parameters)
        {
            try
            {
                DbCommand cmd = DbFactory.CreateDbCommand();
